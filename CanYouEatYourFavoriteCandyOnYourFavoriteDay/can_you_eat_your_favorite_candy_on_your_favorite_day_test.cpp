...
#include "can_you_eat_your_favorite_candy_on_your_favorite_day.h"
...
TEST_F(SolutionTest, case1) {
  ...
  for (size_t i = 0; i < sizeof(test_data)/sizeof(test_data[0]); ++i) {
    ...
    queries.back().push_back(test_data[i].f_type);
    queries.back().push_back(test_data[i].f_day);
    queries.back().push_back(test_data[i].daily_cap);
  }
  ...
  vector<int> candiesCount(a, a + sizeof(a) / sizeof(a[0]));

  bool b[] = {true,true,true,true,true,true,true,true,false,false,true,true,false,false,false,true,true,false,false,false,false,false,false,false,false,true,false,false,false,false,false,true,false,true,true,false,false,false,true,false,false,false,false,false,true,true,true,false,false,false,false,true,false,false,true,false,true,true,false,true,false,false,true,true,true,true,true,false,false,false,true,true,false,false,true,false,true};
  vector<bool> answer = solution_.canEat(candiesCount, queries);
  EXPECT_EQ(answer.size(), sizeof(b) / sizeof(b[0]));
  for (size_t i = 0; i < answer.size(); ++i) {
    EXPECT_EQ(answer[i], b[i]);
  }
}
