...
TEST_F(SolutionTest, case1) {
	...
	vector<vector<int> > vec2d = solution_.levelOrderBottom(root);
	EXPECT_EQ((int)vec2d.size(), 3);
	EXPECT_EQ((int)vec2d[0].size(), 2);
	EXPECT_EQ(vec2d[0][0], 15);
	EXPECT_EQ(vec2d[0][1], 7);
	EXPECT_EQ((int)vec2d[1].size(), 2);
	EXPECT_EQ(vec2d[1][0], 9);
	EXPECT_EQ(vec2d[1][1], 20);
	EXPECT_EQ((int)vec2d[2].size(), 1);
	EXPECT_EQ(vec2d[2][0], 3);
}
